...
@testset "Model" begin
    @testset "default" begin
        ...
        @test isa(model.death_rate_vec, Vector{Float64})
        @test length(model.death_rate_vec) == 100
        @test mean(model.death_rate_vec) ≈ 0.1 atol = 0.01
        @test std(model.death_rate_vec) ≈ 0.005 atol = 0.01
    ...
    @testset "customized" begin
        ...
        @test isa(model.death_rate_vec, Vector{Float64})
        @test length(model.death_rate_vec) == 10_000
        @test mean(model.death_rate_vec) ≈ 1.0 atol = 0.5
...
@testset "interaction!" begin
    @testset "C vs. C" begin
        model = ModelPopulation(Param(initial_N = 2, relationship_volatility = 0.2))
        ...
        # check weight limit
        model = ModelPopulation(Param(initial_N = 2, relationship_volatility = 0.9))
        model.strategy_vec[1] = C
        model.strategy_vec[2] = C
        interaction!(model)
    ...
    @testset "C vs. D" begin
        model = ModelPopulation(Param(initial_N = 2))
        model.strategy_vec[1] = C
    ...
    @testset "D vs. D" begin
        model = ModelPopulation(Param(initial_N = 2, relationship_volatility = 0.3))
        model.strategy_vec[1] = D
        model.strategy_vec[2] = D
        interaction!(model)
    ...
    @testset "Many agents" begin
        model = ModelPopulation(Param(initial_N = 10, rng = MersenneTwister(1)))
    ...
    @testset "Many agents (interaction_freqency = 0.149)" begin
        model = ModelPopulation(Param(initial_N = 10, interaction_freqency = 0.149, rng = MersenneTwister(3)))
        model.strategy_vec = repeat([C, D], 5)
        interaction!(model)
...
@testset "pick_deaths and pick_parents" begin
    ...
    @testset "uniform distribution" begin
        model = ModelPopulation(Param(initial_N = 100))
    ...
    @testset "polarized situation" begin
        model = ModelPopulation(Param(initial_N = 100, δ = 1.0))
...
@testset "death_and_birth!" begin
    @testset "μ = 0.0" begin
        model = ModelPopulation(Param(initial_N = 100, reproduction_rate = 0.03, rng = MersenneTwister(1)))
    ...
    @testset "μ = 1.0" begin
        model = ModelPopulation(Param(initial_N = 100, reproduction_rate = 0.03, μ = 1.0, rng = MersenneTwister(1)))
